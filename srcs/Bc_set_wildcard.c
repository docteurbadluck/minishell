...
/*   Updated: 2025/04/02 16:12:54 by tdeliot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while(ptr && !flag)
	{
		...
		if (temp)
		{
			...
			if (ptr_tracker->redirection)
			{
				while (ptr_tracker->list_of_matched_name[i])
				{
				//	printf(" %s",ptr_tracker->list_of_matched_name[i]);
					i++;
				}
				if (i > 1)
					flag = 1;
...
// work with " "
// work with two wildcard in arg :  work with one wrong one correct. work with two correct 

// 0 argument + > * = segfault not segfault anymore 
