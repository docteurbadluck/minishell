...
void	transform_temp_variable(t_heredoc_manip *heredoc, int y, t_env *head)
{
	int		fd;
	char	*buffer;
	char	*result;
	...
	close(fd);
	result_final = variable_manager(result, head);
...
void	create_temp_files(t_heredoc_manip *heredoc, int nbr_of_heredoc, t_env *head)
{
	int		y;
	char	quote;
	...
	while (y < nbr_of_heredoc)
	{
		...
		if (g_cancel_heredoc)
			break ;
		write(heredoc->fd[y], &quote, 1);
		transform_temp_variable(heredoc, y, head);
		close(heredoc->fd[y]);
...
int	create_heredoc_files(int nbr_of_heredoc,
			t_parsed_command *array_of_cmd, char *argv0, t_env *head)
{
	...
	create_temp_files(&heredoc, nbr_of_heredoc, head);
